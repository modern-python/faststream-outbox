...
- **[F4-01]** `OutboxResponse.__init__` validates `activate_*`/tz eagerly but **not `queue`** (non-str/empty/>255) — a bad `queue` still defers to `as_publish_command()` at dispatch, masquerading as a handler failure and burning the inbound row's retry budget. The exact footgun #83 closed for `activate_*`, still open for `queue`. CONFIRMED. (`response.py:139-162`)
