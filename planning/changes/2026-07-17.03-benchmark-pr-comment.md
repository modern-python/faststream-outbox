  ...
  bench:
    ...
    steps:
      ...
      - name: Run gate, capture markdown
        ...
        run: just bench-report | tee "$GITHUB_STEP_SUMMARY"
      - name: Comment on PR
...
`tee "$GITHUB_STEP_SUMMARY"` puts the table on the run's Summary page for free —
readable even on `push` and fork PRs where no comment posts — and the same file
feeds `--body-file`. `--edit-last --create-if-none` makes the comment sticky
(all three flags verified present in the runner's preinstalled gh 2.96.0).
