...
**Connection budget**: each subscriber holds `max_workers + 1` SQLAlchemy pool connections steady-state (one writer per worker + one fetch), plus one raw asyncpg connection for LISTEN when available. Size the engine pool for `Σ subscribers × (max_workers + 1)` or startup will block on checkout. SQLAlchemy's default `pool_size=5, max_overflow=10` is enough for a handful of single-worker subscribers; bump it for larger fleets.
